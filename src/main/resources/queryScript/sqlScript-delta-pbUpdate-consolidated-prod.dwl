...
---Get LITM from 4106 where BPMCU=1801
    ...
    WHERE trim(X1.BPMCU) IN ('1801','1802') AND trim(X1.BPLITM) IN ($(vars.pricecode)) AND X1.BPEFTJ <= ($(vars.pbJobRun.date)) AND X1.BPEXDJ >= ($(vars.pbJobRun.date))
...
WHERE TRIM(T1.IBMCU) IN ('1801','1802') AND T4.BPLITM IN ($(vars.pricecode)) AND T4.BPEFTJ <= ($(vars.pbJobRun.date)) AND T4.BPEXDJ >= ($(vars.pbJobRun.date))
