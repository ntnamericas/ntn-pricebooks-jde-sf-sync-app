...
BPLITM_F4106 AS (
 ---Get LITM from 4106 where BPMCU=1801
    ...
    ---WHERE  TRIM(X1.BPMCU)='1801' and ((X1.BPEFTJ >= $(vars.previousPbJobRun.date) AND X1.BPEXDJ >= $(vars.previousPbJobRun.date) and X1.BPUPMJ >= $(vars.pbJobRun.date) AND X1.BPTDAY > $(vars.previousPbJobRun.time)) or X1.BPEFTJ >= $(vars.previousPbJobRun.date))
    WHERE (X1.BPEFTJ <= $(vars.pbJobRun.date) AND X1.BPEXDJ >= $(vars.pbJobRun.date)) AND X1.BPLITM = '6004U[T100]' AND (X1.BPMCU = '1801' OR BPMCU = '1802')
),
...
INNER JOIN IBPRP1_IBSRP4_BPLITM T5 ON
   ...
   WHERE TRIM(T1.IBMCU)= ('1801' OR '1802)' AND T4.BPLITM in ($(vars.pricecode))
